    ...
    'tools' => [
        ...
        'code_interpreter' => [
            ...
            'awareness' => implode("\n", [
                ...
                '',
                'FILES OF THE CONVERSATION: the tool definition lists the files of this chat - the uploads of every turn, the images you generated, the decks and plots your code produced - each with its /work/<name>. To work with one, list its name in the `files` argument of the call and read it at /work/<name>; nothing is in /work that you did not list, except the uploads of the newest user message, which are there by themselves. So "put the otter picture on a slide" is ONE call: files: ["otter.png", "deck.pptx"], then Deck.open("/work/deck.pptx").image("Der Otter", "/work/otter.png").save("/tmp/deck.pptx"). A FileNotFoundError under /work means the name was not listed, not that the file is gone. USE a file from /work, never print it back: pass /work/<name> where it belongs (deck.image("<title>", "/work/<name>")). Printing a file that already exists blows the output limit and the run is lost - only a figure your code DRAWS is printed as a data URI.',
                '',
                'PRESENTATIONS: when the user asks for a PowerPoint, a deck or slides, build a real .pptx with the hawki_slides Python module - never with python-pptx or pptxgenjs directly. The whole program, copy this shape and fill in the content: from hawki_slides import Deck; deck = Deck(title="<deck title>", author="HAWKI", lang="de"); deck.title("<title>", "<subtitle>"); deck.bullets("<slide title>", ["<point>", {"text": "<point>", "sub": ["<detail>"]}], notes="<speaker notes>", sources=["<url>"]); deck.cards("<slide title>", [{"heading": "<h>", "text": "<t>"}, {"heading": "<h>", "text": "<t>"}]); deck.two_columns("<slide title>", {"heading": "<left>", "items": ["..."]}, {"heading": "<right>", "items": ["..."]}); deck.quote("<one statement>", "<attribution>"); deck.closing("Danke · Fragen?"); deck.save("/tmp/<title>.pptx") - these are ALL the methods: title, bullets, cards (2-6), two_columns, quote, closing, image(title, "/work/<file>.png|.svg|.jpg", caption), save; every slide method also takes notes="..." and sources=[...], which land in the speaker notes. An image goes on a slide with deck.image(): a picture the image tool generated or a plot your code drew, listed in files so it is at /work/<name>, or an SVG you write to /tmp yourself - SVGs are rasterised for you. To ADD to a deck built earlier, do not rebuild it: deck = Deck.open("/work/<name>.pptx") keeps its slides; add the new ones and save under /tmp again. THE LOOK: by default the deck is on the JLU corporate template - German for lang="de", English for lang="en"; pick lang from the language the user writes in, or the one they ask for. If the user attached a .potx or .pptx, it is their template: Deck(..., template="attached"). Only when the user asks for a neutral or non-JLU look use style="purple" (or blue, green, red, slate). One idea per slide; keep to the number of slides the user asked for (title and closing count). Then check your work in the same run: subprocess.run(["soffice","--headless","--convert-to","pdf","--outdir","/tmp","/tmp/<title>.pptx"], check=True, capture_output=True, text=True), then subprocess.run(["pdftoppm","-png","-r","60","/tmp/<title>.pdf","/tmp/slide"], check=True, capture_output=True, text=True), then for p in sorted(glob.glob("/tmp/slide*.png")): print the file as a data:image/png;base64 URI, so you and the user see all slides (never guess the PNG names, glob them). The .pptx left in /tmp is delivered automatically. If a subprocess fails, print(e) - the message includes the stderr of the program that failed; read it and fix the actual cause. /tmp does not survive between calls, so build, check and print in ONE call.',
                '',
