...
<!-- SHOW API SERVER STATUS  -->
		<a href="https://status.openai.com/" target="_blank"><div id="apiStatus"></div></a>
	...
	async function processStream(stream) {
		...
		try {
			...
			while (true) {
				...
				//console.log(decodedData); // Debugging
				const delimiter = '\n\n';
...
// select code-copybutton
// copyButton.dataset.clicked = "false";
// AddEventListenersToCopyButton(copyButton);
function AddCodeCopyButton() {
    // Finde alle Code-Copy-Buttons
    const copyButtons = document.querySelectorAll('.code-copyButton');
	
    copyButtons.forEach((copyButton) => {
        // Füge Event-Listener hinzu, um den Code zu kopieren, wenn der Button geklickt wird
        copyButton.dataset.clicked = "false";
        AddEventListenersToCopyButton(copyButton);
    });
}
	...
	// Copies content of the message box without the css attributes
		function CopyContentToClipboard(target) {
		    let clone = '';
		    let codeElement = '';
		    // Navigiere zum <code>-Tag innerhalb des <pre>-Tags
		    codeElement = target.closest('pre').querySelector('code');
		
		    if (codeElement) {
		        // Klone den Inhalt des <code>-Tags
		        clone = codeElement.cloneNode(true);
		    } else {
		        console.error('No <code> tag found to copy');
		    }
...
//		const clone = parentElement.cloneNode(true);
	...
	async function send_feedback(){
		...
		let message = {};
		message.role =  '<?= htmlspecialchars($_SESSION['initials']) ?>';
	...
	function fetchAPIStatus() {
            fetch('https://status.openai.com/api/v2/components.json')
                ...
                .then(data => {
                    ...
                    container.textContent = `${apiComponent.name} Status: ${apiComponent.status}`;
                    // Klassenname basierend auf dem Status zuweisen
                    container.className = apiComponent.status.replace(' ', '_').toLowerCase(); // Ersetzt Leerzeichen durch Unterstriche und konvertiert zu Kleinbuchstaben
                })
                .catch(error => {
                    console.error('Fehler beim Abrufen der Daten:', error);
                    const container = document.getElementById('apiStatus');
                    container.textContent = 'Fehler beim Laden des Status';
                    container.className = 'error'; // Setzt den Fehlerstatus, falls ein Fehler beim Abruf auftritt
                });
        }
