...
echo "   Using template: nginx.template.${DEPLOY_PROFILE}"
    ...
    # Setup S3 Endpoint Variables
    export S3_ENDPOINT_URL="${S3_ENDPOINT:-http://host.docker.internal:9000}"
    export S3_ENDPOINT_HOST="${S3_ENDPOINT_URL#*://}"
    ...
    sed -e "s|\${NGINX_SERVER_NAME}|${NGINX_SERVER_NAME}|g" \
        ...
        -e "s|\${S3_ENDPOINT_URL}|${S3_ENDPOINT_URL}|g" \
        -e "s|\${S3_ENDPOINT_HOST}|${S3_ENDPOINT_HOST}|g" \
