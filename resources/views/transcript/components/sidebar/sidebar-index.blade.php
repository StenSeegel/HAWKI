    ...
    <div class="dy-sidebar-wrapper">
        <div class="header">
            <button id="new-transcription-btn" class="btn-md-stroke" onclick="showTranscriptChoice()">
                ...
                <div class="label"><strong>{{ $translation["TranscriptStartNew"] ?? 'Neue Transkription starten' }}</strong></div>
            </button>
            <h3 class="title">{{ $translation["TranscriptSidebarTitle"] ?? 'Transkription' }}</h3>
        </div>
            ...
            <div class="dy-sidebar-scroll-panel">
                <div id="file-transcription-options" class="hidden" style="padding: 15px;">
                    <div class="transcript-sidebar-field border-label-field">
                        <label for="language-select">{{ $translation["TranscriptLanguage"] ?? 'Sprache' }}</label>
                        <div class="select-wrapper">
                            <x-icon name="world" class="field-icon" />
                            <select id="language-select" name="language">
                                <option value="auto">Auto</option>
                                <option value="de">{{ $translation["TranscriptLanguageGerman"] ?? 'Deutsch' }}</option>
                                <option value="en">{{ $translation["TranscriptLanguageEnglish"] ?? 'Englisch' }}</option>
                            </select>
                        </div>
                    </div>

                    <div class="transcript-sidebar-field border-label-field">
                        <label for="speaker-select">{{ $translation["TranscriptDetectSpeakers"] ?? 'Sprecher*innen erkennen' }}</label>
                        <div class="select-wrapper">
                            <x-icon name="users" class="field-icon" />
                            <select id="speaker-count" name="speaker_count">
                                <option value="auto">Auto</option>
                                <option value="single">{{ $translation["TranscriptSingleSpeaker"] ?? 'Einzelne Person' }}</option>
                                <option value="multi">{{ $translation["TranscriptMultipleSpeakers"] ?? 'Mehrere Personen' }}</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="transcript-sidebar-checkboxes" style="margin-top: 15px; margin-bottom: 5px;">
                        <label class="custom-checkbox">
                            {{ $translation["TranscriptOptimizeSpeakersAI"] ?? 'Sprecher per KI optimieren' }}
                ...
                <div id="live-record-sidebar-options" class="hidden" style="padding: 15px;">
                    <div class="transcript-sidebar-field border-label-field">
                        <label>{{ $translation["TranscriptStatusLabel"] ?? 'Status' }}</label>
                        <div id="live-record-status-container-sidebar" class="live-record-status-box">
                            ...
                            <div>
                                <div id="live-record-status-title-sidebar" class="live-record-status-box-title">{{ $translation["TranscriptRecordingReady"] ?? 'Aufnahme bereit' }}</div>
                                <div id="live-record-status-text-sidebar" class="live-record-status-box-text">{{ $translation["TranscriptSelectMicrophoneHint"] ?? 'Wählen Sie ein Mikrofon und starten Sie die Aufnahme.' }}</div>
                            </div>
                        </div>
                    </div>

                    <div class="transcript-sidebar-field border-label-field" style="margin-top: 20px;">
                        <label for="live-input-device-select-sidebar">{{ $translation["TranscriptMicrophone"] ?? 'Mikrofon' }}</label>
                        <div class="select-wrapper">
                            <x-icon name="microphone" class="field-icon" />
                            <select id="live-input-device-select-sidebar">
                                <option value="">{{ $translation["TranscriptSearchingDevices"] ?? 'Suche Geräte...' }}</option>
                            </select>
                        </div>
                    </div>
                </div>
                ...
                <div id="live-transcript-sidebar-options" class="hidden" style="padding: 15px;">
                    <div class="transcript-sidebar-field border-label-field">
                        <label for="live-transcript-mode-select">{{ $translation["TranscriptModeLabel"] ?? 'Modus' }}</label>
                        <div class="select-wrapper">
                            ...
                            <select id="live-transcript-mode-select" name="live_transcript_mode">
                                @foreach (['onprem' => $translation["TranscriptModeLocal"] ?? 'Lokal (Standard)', 'openai' => 'OpenAI'] as $modeKey => $modeLabel)
                    ...
                    <div class="transcript-sidebar-field" style="margin-top: 15px;">
                        ...
                        <div style="margin-bottom: 20px;">
                            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px;">
                                <span class="live-option-label">{{ $translation["TranscriptFontSize"] ?? 'Schriftgröße' }}</span>
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center;">
                            <span class="live-option-label">{{ $translation["TranscriptInvertContrast"] ?? 'Kontrast umkehren' }}</span>
                ...
                <div id="sidebar-history-content">
                    <div class="sidebar-search-container">
                        <div class="search-input-wrapper">
                            ...
                            <input type="text" id="history-search" placeholder="{{ $translation["TranscriptSearchPlaceholder"] ?? 'Suche Transkriptionen' }}"
