...
<template id="tmpl-export-check-data">
    <div class="export-state-container export-state-check">
        ...
        <p class="export-state-text">{{ $translation["TranscriptExportCheckingData"] ?? 'Prüfe Daten...' }}</p>
    </div>
</template>
    ...
    <div class="export-loading-container">
        ...
        <div class="export-loading-banner">
            ...
            <div class="export-loading-banner-right">
                <h4 class="export-loading-banner-title">{{ $translation["TranscriptExportGeneratingSummary"] ?? 'Zusammenfassung wird erstellt' }}</h4>
                <p class="export-loading-banner-text">{{ $translation["TranscriptExportGeneratingSummaryHint"] ?? 'Je nach Länge des Transkripts dauert das einen Moment.' }}</p>
            </div>
        </div>
...
<template id="tmpl-export-error">
    <div class="export-state-container export-state-error">
        ...
        <p class="export-state-title">{{ $translation["TranscriptExportGenerationFailed"] ?? 'Generierung fehlgeschlagen' }}</p>
        <p class="export-state-subtext error-msg"></p>
        <button class="btn-sidebar-action" onclick="window.generateErgebnisprotokoll()" style="margin-top: 15px;">{{ $translation["TranscriptRetry"] ?? 'Erneut versuchen' }}</button>
    </div>
</template>
...
<template id="tmpl-export-generate-prompt">
    <div class="export-state-container export-state-generate">
        <h3 class="export-state-title">{{ $translation["TranscriptExportGenerateSummaryTitle"] ?? 'Ergebnisprotokoll generieren' }}</h3>
        <p class="export-state-subtext">{{ $translation["TranscriptExportGenerateSummaryDesc"] ?? 'Erstelle eine KI-gestützte Zusammenfassung des aktuellen Transkripts. Dieser Vorgang dauert etwa 10-20 Sekunden.' }}</p>
        <button onclick="window.generateErgebnisprotokoll()" class="btn-primary-blue">{{ $translation["TranscriptExportGenerateNow"] ?? 'Jetzt generieren' }}</button>
    </div>
</template>
...
<template id="tmpl-ui-btn-saving">
    <div class="loader-spinner"></div> {{ $translation["TranscriptSaving"] ?? 'Speichern...' }}
</template>

{{-- Transcript UI: Saved Button --}}
<template id="tmpl-ui-btn-saved">
    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg> {{ $translation["TranscriptSaved"] ?? 'Gespeichert' }}
</template>
...
<template id="tmpl-context-edit">
    <div class="transcript-context-menu-item" id="context-edit">
        ...
        {{ $translation["TranscriptEditText"] ?? 'Text bearbeiten' }}
    </div>
</template>
...
<template id="tmpl-context-redact">
    <div class="transcript-context-menu-item" id="context-redact">
        ...
        {{ $translation["TranscriptRedactBracketed"] ?? '[Ausblenden]' }}
    </div>
</template>
...
<template id="tmpl-context-unredact">
    <div class="transcript-context-menu-item" id="context-unredact">
        ...
        {{ $translation["TranscriptUnredactBracketed"] ?? '[Einblenden]' }}
    </div>
</template>

{{-- Segment Processor: Sentence Reorder Toolbar --}}
<template id="tmpl-sentence-reorder">
    <button class="reorder-btn move-up" title="{{ $translation["TranscriptMoveSentenceUp"] ?? 'Satz nach oben verschieben' }}">
    ...
    <button class="reorder-btn move-down" title="{{ $translation["TranscriptMoveSentenceDown"] ?? 'Satz nach unten verschieben' }}">
    ...
    <button class="reorder-btn close-toolbar" title="{{ $translation["TranscriptClose"] ?? 'Schließen' }}">
